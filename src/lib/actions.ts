...
import { aiAssistedTransactionTroubleshooting } from "@/ai/flows/ai-assisted-transaction-troubleshooting";
...
export async function updateTransactionStatus(
  ...
  try {
    ...
    } else if (response.errCode === "997" || response.errCode === "999") {
      console.log(
        `UNKNOWN STATUS: Transaction ${transaction.referenceid} returned ${response.errCode}.`
      );
      
      const transactionDetails = JSON.stringify(transaction, null, 2);
      
      const aiResponse = await aiAssistedTransactionTroubleshooting({
        errCode: response.errCode,
        transactionDetails: transactionDetails,
      });

      return {
        aiAssistance: aiResponse.troubleshootingSteps,
      };
    } else {
      console.log(
